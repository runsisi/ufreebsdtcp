...
#include <sys/cdefs.h>
__FBSDID("$FreeBSD: release/9.2.0/sys/kern/kern_ntptime.c 225617 2011-09-16 13:58:51Z kmacy $");
...
static bsd_int64_t time_adjtime;		/* correction from adjtime(2) (usec) */
...
static struct bsd_timespec pps_tf[3];	/* phase median filter */
...
{
	struct bsd_timespec atv;	/* nanosecond time */
 ...
 */
void
ntp_update_second(bsd_int64_t *adjustment, bsd_time_t *newsec)
{
	int tickrate;
...
void
hardpps(tsp, nsec)
	struct bsd_timespec *tsp;	/* time at PPS */
...
{
	struct bsd_timeval delta, olddelta, *deltap;
	int error;
...
int
kern_adjtime(struct thread *td, struct bsd_timeval *delta, struct bsd_timeval *olddelta)
{
	...
	if (delta) {
		...
		time_adjtime = (bsd_int64_t)delta->tv_sec * 1000000 +
		    delta->tv_usec;
	}
	mtx_unlock(&Giant);
	return (0);

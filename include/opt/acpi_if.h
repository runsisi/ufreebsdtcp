...
typedef ACPI_STATUS (*acpi_scan_cb_t)(ACPI_HANDLE h, bsd_device_t *dev,
    int level, void *arg);
...
typedef char * acpi_id_probe_t(bsd_device_t bus, bsd_device_t dev, char **ids);

static __inline char * ACPI_ID_PROBE(bsd_device_t bus, bsd_device_t dev, char **ids)
{
	kobjop_t _m;
...
typedef ACPI_STATUS acpi_evaluate_object_t(bsd_device_t bus, bsd_device_t dev,
                                           ACPI_STRING pathname,
                                           ACPI_OBJECT_LIST *parameters,
                                           ACPI_BUFFER *ret);

static __inline ACPI_STATUS ACPI_EVALUATE_OBJECT(bsd_device_t bus, bsd_device_t dev,
                                                 ACPI_STRING pathname,
                                                 ACPI_OBJECT_LIST *parameters,
                                                 ACPI_BUFFER *ret)
{
...
typedef int acpi_pwr_for_sleep_t(bsd_device_t bus, bsd_device_t dev, int *dstate);

static __inline int ACPI_PWR_FOR_SLEEP(bsd_device_t bus, bsd_device_t dev, int *dstate)
{
	kobjop_t _m;
...
typedef ACPI_STATUS acpi_scan_children_t(bsd_device_t bus, bsd_device_t dev,
                                         int max_depth, acpi_scan_cb_t user_fn,
                                         void *arg);

static __inline ACPI_STATUS ACPI_SCAN_CHILDREN(bsd_device_t bus, bsd_device_t dev,
                                               int max_depth,
...
typedef int acpi_ec_read_t(bsd_device_t dev, u_int addr, UINT64 *val, int width);

static __inline int ACPI_EC_READ(bsd_device_t dev, u_int addr, UINT64 *val,
                                 int width)
{
	kobjop_t _m;
...
typedef int acpi_ec_write_t(bsd_device_t dev, u_int addr, UINT64 val, int width);

static __inline int ACPI_EC_WRITE(bsd_device_t dev, u_int addr, UINT64 val,
                                  int width)
{
	kobjop_t _m;
...
typedef int acpi_batt_get_info_t(bsd_device_t dev, struct acpi_bif *bif);

static __inline int ACPI_BATT_GET_INFO(bsd_device_t dev, struct acpi_bif *bif)
{
	kobjop_t _m;
...
typedef int acpi_batt_get_status_t(bsd_device_t dev, struct acpi_bst *bst);

static __inline int ACPI_BATT_GET_STATUS(bsd_device_t dev, struct acpi_bst *bst)
{
	kobjop_t _m;
